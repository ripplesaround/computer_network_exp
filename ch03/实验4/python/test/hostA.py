...
class time_ack:
    ...
    def run(self,n):
        ...
        while self._running:
            now = time.time()
            if now - self.current_time >2:
                self.current_time = now
                if flag_ack:
                    print("发送一个确认帧")
                    client.sendto('ack'.encode('utf-8'),target)
                    flag_ack = False
...
from_client = client.recvfrom(1024)
print(f"来自{from_client[1]}的消息：{from_client[0].decode('utf-8')}")
client.sendto(str(from_client[1][1]).encode('utf-8'),from_client[1])

send_ack = time_ack()
send_ack_t = threading.Thread(target=send_ack.run, args=(10,))
send_ack_t.start()
...
while 1:
    ...
    if flag_control == 1:
        ...
        if np.random.randint(0, FilterLost) > 0:
            client.sendto(se, ('127.0.0.1', 9999))
            current_time = time.time()
