...
public class CRC {
	...
	public static BigInteger getbi2(String linestr) {//将二进制字符串转为biginteger
		String bs = linestr.substring(linestr.indexOf("=") + 1);
		BigInteger ib = new BigInteger(bs, 2);
		return ib;
	}
	
	public static BigInteger getbi10(String linestr) {//将十进制字符串转为biginteger
		String bs = linestr.substring(linestr.indexOf("=") + 1);
		BigInteger ib = new BigInteger(bs, 10);
		return ib;
	}
